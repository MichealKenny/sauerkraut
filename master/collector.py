...
while True:
    ...
    for server in servers:
        try:
            health = get('https://{host}:{port}/extended'.format(host=server[1], port=server[2]), timeout=0.01, verify=False).json()
    ...
    log.execute("DELETE FROM servers WHERE ROWID IN (SELECT ROWID FROM servers ORDER BY"
                " ROWID DESC LIMIT -1 OFFSET {0})".format(int(len(servers)) * 120))
